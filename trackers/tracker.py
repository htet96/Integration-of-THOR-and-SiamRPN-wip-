...
import torch
from trackers.THOR_modules.wrapper import THOR_SiamFC, THOR_SiamRPN, THOR_SiamMask, THOR_SiamRPNpp
...
# SiamRPNpp Imports
from trackers.SiamRPNpp.models.model_builder import ModelBuilder
# from trackers.SiamRPNpp.tracker.siamrpn_tracker import SiamRPNTracker
from trackers.SiamRPNpp.tracker.siamrpn_tracker import SiamRPNpp_init, SiamRPNpp_track
...
class SiamRPNpp_Tracker(Tracker):
    def __init__(self, cfg):
        super(SiamRPNpp_Tracker, self).__init__()
        self.cfg = cfg

        # setting up the model
        model_path = dirname(abspath(__file__)) + '/SiamRPNpp/model.pth'
        model = ModelBuilder()
        model.load_state_dict(torch.load(model_path, map_location=('cpu'
                   if str(self.device) == 'cpu' else None)), strict=False)
        self.model = model.eval().to(self.device)

        # set up template memory
        self.temp_mem = THOR_SiamRPNpp(cfg=cfg['THOR'], net=self.model)

    def init_func(self, im, pos, sz):
        return SiamRPNpp_init(im, pos, sz, self.cfg['tracker'])

    def track_func(self, state, im):
        return SiamRPNpp_track(state, im, self.temp_mem)
